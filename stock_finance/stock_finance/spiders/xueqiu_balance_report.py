...
class XueQiuBalanceReportSpider(scrapy.Spider):
    '''
    雪球年报爬虫
    '''
    name = 'xueqiu_balance_report'
    #allowed_domains = ['*.xueqiu.com']
    start_urls = ['https://stock.xueqiu.com/']
    download_delay = 5

    def __init__(self, *args, **kwargs):
        super(XueQiuBalanceReportSpider, self).__init__(*args, **kwargs)
    ...
    def parse_balance_report(self, response):
    ...
    def parse(self, response):
        if response.status == 200:
            url = 'https://stock.xueqiu.com/v5/stock/finance/cn/balance.json?symbol=%s&type=Q4&is_detail=true&count=10&timestamp=' % self.security_code
            yield Request(url, callback=self.parse_balance_report, dont_filter=True)
